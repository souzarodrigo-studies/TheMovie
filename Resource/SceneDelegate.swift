...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let rootModule = container.resolve(HomeBuilder.self) {
...
extension SceneDelegate {
    ...
    /// Metodo responsavel para configurar todas as classes de dependencias
    func setupDI() {
        
        // MARK: - Presentation
        // Criando factories das instâncias de apresentação que serão utilizadas no app
        
        container.register(HomeBuilder.self) {
            resolver in
            return HomeBuilder.build(with: self.container)
        }
